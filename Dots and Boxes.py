    ...
    #If mouse is clicked, draws a line, if not already drawn and is within the board.
        if pygame.mouse.get_pressed()[0] and not alreadyPlaced and not outOfBounds:
            ...
            if self.turn == False and self.player2 == 'AI':
                ...
                pass
        #Calld function to draw the boxes with every display flip.
        for x in range (5):
            for y in range (5):
                self.drawBox(x,y)
...
#Function create the interface of the scoreboard and turns.    
    def drawScoreBoard(self):
        ...
        fontTurn = pygame.font.SysFont("forte", 32)
        ...
        label = fontTurn.render("Turn:", 1, (0,0,200))
        player = fontTurn.render(self.playerTurn, 1, (0,0,0))
        self.screen.blit(label, (10, 540))
        self.screen.blit(player, (100, 540))

        #Defining the fonts for the names and scores.
        fontScore = pygame.font.SysFont("forte", 40)
        fontName = pygame.font.SysFont("forte", 20)

        #Creating the text for turns and their scores.
        #Player text colored according to chosen color. 
        scoreOne = fontScore.render(str(self.playerOneScore), 1, (0,0,0))
        scoreTwo = fontScore.render(str(self.playerTwoScore), 1, (0,0,0))
        scoreOneText = fontName.render(str(self.player1), 1, self.textColor[self.color1])
        scoreTwoText = fontName.render(str(self.player2), 1, self.textColor[self.color2])
